...
| `docs/設計書/技術負債と将来課題.md` | **複数 Phase 横断の改善案・将来課題の集約**（UX / パフォーマンス / 保守性 / インフラ） | 中 |
...
`docs/設計書/技術負債と将来課題.md` は **複数 Phase 横断の課題集約所**。以下のタイミングで必ず参照・更新する:

| タイミング | アクション | 担当 Skill |
|----------|---------|----------|
| 新 Phase の Stage 1 設計時 | 既存課題から取り込むべきものを判断、Phase スコープに組み込めないか検討 | `/new-feature` |
| 設計レビュー時 | 既存課題と矛盾しないか / 解消できる設計になっていないか確認 | `/design-review` |
| コードレビュー時 | 既存課題と照合し、レビュー指摘の重複や関連を発見 | `/code-review` |
| `/update-docs` 実行時 | 「次フェーズ検討」「許容（将来対応）」とした項目で**複数 Phase 横断のもの**を本ドキュメントへ転記 | `/update-docs` |

**Phase 固有の改善案** は機能設計書 §7「将来タスク」に書く。本ドキュメントは横断のみ。
...
1. **バックアップ実行**: スキーマ変更の**前に** `npm run db:backup` を実行（内部で `tsx --env-file=.env.local tools/export-to-sql.ts` が走る）
   - 直接 `npx tsx tools/export-to-sql.ts` は **使わない**。`.env.local` が読まれず DATABASE_URL 未取得で空エクスポートになる
   - 初回マイグレーション (空 DB) でも `ORDERED_TABLES` を更新済みなら実行する。データなしでも空 SQL が生成され実行成功扱い
2. **コメント必須**: カラム追加・変更時は `/// 説明` コメントを必ず付与する（テーブル定義書の自動生成に使用）
3. **3点同期**: スキーマ変更時は以下の3箇所を**必ず同時に更新**する:

| # | 対象 | ファイル | コマンド |
|---|------|---------|---------|
| 1 | スキーマ | `prisma/schema.prisma` | (手動編集) |
| 2 | 設計書 | `docs/設計書/テーブル定義書.md` | `npm run db:gen-docs` |
| 3 | バックアップツール | `tools/export-to-sql.ts`（`ORDERED_TABLES` + `DB_TABLE_MAP`） | (手動編集) |
