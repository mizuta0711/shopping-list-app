...
| 削除 UI | `window.confirm` で確認後、ストアから削除（既存 `SettingsView.handleReset` と同じ流儀） |
| 購入済みアイテムへの適用 | 購入済みアイテムも編集・削除できる。**履歴画面（/history）にもスワイプアクションを適用**（誤って購入済みにしたアイテムを未購入に戻すボタンと、削除ボタンを行末に出す）|
| Phase 10.3 の編集アイコン方針との関係 | Phase 10.3 の「カテゴリ別表示モード時のみ行末に編集アイコン」方針は本フェーズで**不要化**される。Phase 10.3 着手時に方針を見直す（注意事項に記載） |
| ストア変更 | `useShoppingStore` に **`updateItemName(id, name)`** を新規追加（`updatedAt` も更新）。Phase 10.1b でクラウド同期に乗せるとき LWW で機能する前提 |
...
| # | フェーズ | タスク | ステータス | 備考 |
|---|---------|--------|-----------|------|
| 1 | Stage 1 | Stage 1 機能・画面設計の記入 | 🟡 レビュー中 | 本書 §3 |
| 2 | Stage 1 | `/design-review feature` 実行（code-reviewer + product-advisor） | 🔵 未実施 | |
| 3 | Stage 1 | レビュー指摘の反映 → Stage 1 確定 | 🔵 未実施 | |
| 4 | Stage 2 | Stage 2 技術設計の記入 | 🔵 未実施 | |
| 5 | Stage 2 | `/design-review tech` 実行（code-reviewer） | 🔵 未実施 | |
| 6 | 実装 | `@use-gesture/react` 導入 + 共通 `SwipeableRow` ラッパー作成 | 🔵 未実施 | |
| 7 | 実装 | `useShoppingStore.updateItemName` アクション追加 | 🔵 未実施 | `updatedAt` 同期 |
| 8 | 実装 | `SortableItemRow` を SwipeableRow + ドラッグハンドル分離方式に改修 | 🔵 未実施 | |
| 9 | 実装 | アイテム編集モーダル（名前のみ） | 🔵 未実施 | OnboardingModal の overlay 流儀 |
| 10 | 実装 | 履歴画面 (`/history`) のアイテム行にもスワイプアクション適用（未購入に戻す + 削除）| 🔵 未実施 | |
| 11 | 検証 | code-review / browser-test / build-check / update-docs | 🔵 未実施 | |
...
- [ ] 60px 以上スワイプして指を離すとボタン位置にロックされる
- [ ] 60px 未満で指を離すとアニメーションで自動的に閉じる
- [ ] 「編集」タップでモーダルが開き、名前を編集して保存できる
- [ ] 編集後、メイン画面の表示も更新される
- [ ] 「削除」タップで確認ダイアログが出て、OK でリストから消える
- [ ] 行末の `≡` ハンドル長押しで縦ドラッグ並び替えができる
- [ ] **行本体（`≡` 以外）を長押ししても並び替えにならない**
- [ ] アイテムタップで購入済みトグルは引き続き動作する
- [ ] 別の行をスワイプ・タップすると、開いていた行は自動的に閉じる
- [ ] 履歴画面でも同じスワイプ UI が動作する（「未購入に戻す / 削除」）
- [ ] 縦スクロール中に誤って横スワイプが発動しない（`axis: 'x'` の効果）
- [ ] **iOS Safari のブラウザ「戻る」ジェスチャー（左端からのスワイプ）と干渉しない**
...
- 既存の「復元（未購入に戻す）」単発ボタンを**廃止**
- 各行を左にスワイプ → 「**未購入に戻す** / **削除**」の 2 ボタン
- 「未購入に戻す」 → 既存の購入済みトグルを呼ぶ（PURCHASED → PENDING）
- 「削除」 → 確認 → `deleteItem`

メイン画面と履歴画面で**同じ `SwipeableRow` ラッパーコンポーネント**を使い、行末アクションを props で差し替える設計とする（詳細は Stage 2）。
...
| 観点 | 対応 |
|------|------|
| キーボード操作 | スワイプはタッチ前提だが、**`≡` ハンドルにフォーカス → Enter キーで編集モーダルを直接開く**ショートカットを補助として用意（モーダルからは Tab で「キャンセル / 保存」へ移動可能） |
| スクリーンリーダー | 行に `aria-label="{name}（タップで購入済み、左スワイプで編集・削除）"` のヒントを追加 |
| タッチターゲット | 行末アクション（編集・削除）は各 60px 幅 × 行高（44px 以上を維持） |
...
⚪ Stage 1 確定後に着手する。記入内容は以下の項目を予定:

- `@use-gesture/react` の `useDrag` 設定（`axis: 'x'`, `filterTaps: true`, `lockDirection`, `bounds` 等の詳細）
- `SwipeableRow` 共通コンポーネントの props 設計（actions: Array<{label, onAction, color}>）
- `SortableItemRow` のリファクタ（`{...listeners}` を `≡` ハンドル要素のみに移動）
- `openSwipeId` の管理場所（`ShoppingMainView` / `HistoryView` ローカル state）
- 編集モーダル（`ItemEditModal`）の実装パターン
- `useShoppingStore.updateItemName(id, name)` アクションの追加
- 履歴画面の既存「復元ボタン」廃止と `SwipeableRow` への置き換え
- 削除確認・トースト文言・エラーハンドリング
- ブラウザ評価計画 §5 の詳細化
...
| 版数 | 日付 | コミット | 内容 | 担当 |
|------|------|---------|------|------|
| 1.0 | 2026-05-05 | (未確定) | 初版ドラフト作成（Stage 1 機能・画面設計）。横スワイプ + ドラッグハンドル分離方式を採用、`@use-gesture/react` を選定 | Claude Code |
