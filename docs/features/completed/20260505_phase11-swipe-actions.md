...
| 2.0 | 2026-05-05 | e8e5c05 | **巻き戻し**: 実機での操作感が悪く動作不安定（横ジェスチャ判定の不安定性、dnd-kit との競合）のため、ユーザー指示により本フェーズの成果物（`@use-gesture/react` / `SwipeableRow.tsx` / アンドゥトースト / `restoreItem` ストアアクション）を完全撤廃。代替実装として [Phase 13: 編集モード UI](./20260505_phase13-edit-mode.md) を導入。`ItemEditModal` および `updateItemName` は Phase 13 でも継続利用 | Claude Code |
