...
| 12 | Phase 9 | ビルド・型・lint・E2E | ✅ 完了（自動範囲）| `npm run build` + `npm run lint` 通過。E2E は Playwright MCP 未導入のため Phase 9.1 で検討（技術負債と将来課題.md 候補） |
| 13 | Phase 9 | ブラウザ動作確認（複数端末シナリオ） | ✅ 完了（自動範囲）| Playwright MCP で 13/13 合格。OAuth ログイン後の M1-M6 / E1-E6 は実機 Google アカウント必須のため §7「将来タスク - 手動テスト積み残し」に記録、リリース前に手動消化 |
