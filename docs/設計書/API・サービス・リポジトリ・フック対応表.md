...
| 1.3 | 2026-05-06 | (未確定) | Phase 10.4 で sets API 2 本 (PUT /api/sync/sets, POST /api/sync/sets/merge) の使用ヘルパーに `ensureUnclassifiedList` を追記、データアクセスに `listId 補完` を追記、関連フックに `useSetsStore` / `useListsStore` を追加 | Claude Code |
