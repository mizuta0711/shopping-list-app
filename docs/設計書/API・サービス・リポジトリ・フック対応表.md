...
| `/api/sync/merge` | POST | `requireSession`, `SyncMergeSchema`, `toDTO` | `prisma.$transaction` (shoppingItem upsert + 全件再取得) | (未実装、タスク #11 useInitialMerge → `syncClient.mergeOnLogin()`) | Phase 9 |
