...
| 1.3 | 2026-05-06 | (未確定) | Phase 10.4 で sets API に `listId` フィールドを追加。`GET /api/sync/sets` レスポンスに `listId` を反映。`PUT /api/sync/sets` の Zod スキーマに `listId: optional` を追加（旧クライアント後方互換）、サーバー側で未指定 / 不正 listId を未分類補完するロジックを追記。`POST /api/sync/sets/merge` のリクエスト / レスポンス例を `listId` 含む形に更新。リスト削除 tx 内で所属セットの `listId` も未分類へ書き換える連鎖処理を `PUT /api/sync/lists` に追加 | Claude Code |
